...
def predict(path, model, output_path, device, use_tiling):
    ...
    if use_tiling:
        ...
        padding = {"x": 16, "y": 16}
        pred = predict_with_padding(pp, [input_], padding=padding)[0]
...
def main():
    ...
    parser.add_argument("--min_size", help="minimal segment size", default=100)
    ...
    parser.add_argument("-t", "--tiling", type=int, help="Use tiling in prediction", default=0)
    args = parser.parse_args()
    if args.output_path is None:
        output_path = os.path.splitext(args.path)[0] + ".h5"
    else:
        output_path = args.output_path
    image, foreground, affinities, offsets = predict(args.path, args.model, output_path, args.device, args.tiling)
